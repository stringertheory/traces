...
def test_iterperiods():
    ...
    for t0, t1, v0 in ts.iterperiods(
        start=datetime.datetime(2015, 3, 1), end=datetime.datetime(2015, 3, 4)
    ):
    ...
    def filter_function(t0, t1, value):
...
def test_slice():
    ...
    assert list(ts.slice(0.5, 2.5).items()) == [(0.5, 1), (1, 5), (2.5, 5)]
    assert list(ts.slice(1.0, 2.5).items()) == [(1.0, 5), (2.5, 5)]
