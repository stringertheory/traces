...
for hour in business_hours:
    ...
    print(f"{hour.hour:02d}:00 - {hour.hour:02d}:59: {logins_this_hour} logins (Total: {total_logins})")
...
session_durations = [
    timedelta(minutes=random.randint(30, 90)) for _ in range(len(website_logins))
]
...
# Analyze intervals
average_interval = sum([interval.total_seconds() for interval in login_intervals]) / len(login_intervals)
...
if morning_intervals:
    print(f"Morning average interval: {sum(morning_intervals)/len(morning_intervals):.1f} minutes")
if afternoon_intervals:
    print(f"Afternoon average interval: {sum(afternoon_intervals)/len(afternoon_intervals):.1f} minutes")
...
def hourly_counts(event_series, start_time, end_time):
    ...
    while current < end_time:
        ...
        count = event_series.events_between(current, next_hour - timedelta(seconds=1))
...
if clusters:
    ...
    for start_time, end_time, count in clusters:
        ...
        print(f"  {start_time.strftime('%H:%M:%S')} - {end_time.strftime('%H:%M:%S')}: " 
