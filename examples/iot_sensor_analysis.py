...
def generate_sensor_data(duration_days=3, avg_readings_per_day=48, 
                         base_value=20, noise_level=2, 
                         missing_data_prob=0.1, anomaly_prob=0.05):
    """
    ...
    for i in range(n_readings):
        ...
        base_temp = base_value + (daily_factor * daily_amplitude) + weekend_boost
...
temp_outside = generate_sensor_data(base_value=15, noise_level=3.0, 
                                   avg_readings_per_day=24)
...
temp_diff = traces.TimeSeries.merge(
    ...
    operation=lambda values: values[0] - values[1] if None not in values else None
)
...
def detect_threshold_crossings(ts, threshold, rising=True):
    ...
    for time, value in ts:
        if prev_value is not None and value is not None:
            if rising and prev_value < threshold and value >= threshold:
                crossings.append((time, value))
            elif not rising and prev_value > threshold and value <= threshold:
                crossings.append((time, value))
...
def detect_anomalies(ts, window_size=timedelta(hours=6), z_score_threshold=3):
    ...
    for i, (current_time, current_value) in enumerate(measurements):
        ...
        for time, value in measurements:
            if current_time - window_size <= time < current_time and value is not None:
        ...
        window_std = (sum((v - window_mean) ** 2 for v in window_values) / len(window_values)) ** 0.5
...
for i, (time, temp, z_score) in enumerate(sorted(anomalies, key=lambda x: x[2], reverse=True)):
    if i < 5:  # Show top 5 anomalies by z-score
        print(f"{time.strftime('%Y-%m-%d %H:%M:%S')}: {temp:.1f}°C (z-score: {z_score:.2f})")
...
start_time = min(temp_kitchen.first_key(), temp_living_room.first_key(), temp_outside.first_key())
end_time = max(temp_kitchen.last_key(), temp_living_room.last_key(), temp_outside.last_key())
times, resampled = resample_all_sensors(start_time, end_time, timedelta(hours=1))
...
for i, t in enumerate(times):
    if i < 5:  # Show only first 5 hours
        k_val = "None" if resampled["Kitchen"][i] is None else f"{resampled['Kitchen'][i]:.1f}°C"
        l_val = "None" if resampled["Living Room"][i] is None else f"{resampled['Living Room'][i]:.1f}°C"
        o_val = "None" if resampled["Outside"][i] is None else f"{resampled['Outside'][i]:.1f}°C"
        print(f"{t.strftime('%Y-%m-%d %H:%M:%S')} | {k_val:10} | {l_val:11} | {o_val:10}")
...
for i, (start, end, is_freezing) in enumerate(freezing_mask.iterperiods(value=True)):
    if i < 3:  # Show only first 3 periods
        duration = (end - start).total_seconds() / 3600  # Hours
        print(f"From {start.strftime('%Y-%m-%d %H:%M:%S')} to {end.strftime('%Y-%m-%d %H:%M:%S')} ({duration:.1f} hours)")